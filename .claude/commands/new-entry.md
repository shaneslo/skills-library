...
argument-hint: <id-slug> "<human name>"
---

Create a new asset entry. Use the first argument as the id slug and the filename, and the second as the name when given.

Write a new file at `content/entries/$1.yaml` from the scaffold below. Fill what the request gives you and leave the rest as the prompts shown. Match the depth of the exemplar in `SKILLS_LIBRARY_SPEC.md` section 8. An entry that cannot pass the three tests visibly is not ready, so do not scaffold and walk away; load `.claude/skills/tax-ops-domain.md` and write a real `domain_gap`.

For a prompt, skill, or agent, use this scaffold:

```yaml
id: $1                       # stable slug, lowercase-with-hyphens, matches the filename
